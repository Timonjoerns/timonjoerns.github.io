...
async function createFloaty(texture, url, hoverText) {
    ...
    sprite.hoverProgress = 0; // smooth hover easing
...
function drawCircularText(container, text, radius, color = 0xffffff) {
    ...
    const charSize = Math.min(24, radius * 0.15); // scale font size for smaller radius
    for (let i = 0; i < len; i++) {
...
app.ticker.add(() => {
    ...
    floaties.forEach(f => {
        ...
        f.hoverProgress += (targetHover - f.hoverProgress) * 0.1;

        // Apply hover effects
        const hoverScale = f.baseScale * (1 + 0.4 * f.hoverProgress);
        f.scale.set(hoverScale);
        f.alpha = 0.6 + 0.4 * f.hoverProgress;
        f.currentRotationSpeed = f.baseRotationSpeed * (1 - f.hoverProgress);
        const targetRot = hovering ? 0 : f.targetRotation;
        f.rotation += (targetRot - f.rotation) * 0.05 * f.hoverProgress;

        // Hover text
        if (f.hoverProgress > 0.1) {
            if (!f.hoverColor) {
            ...
            drawCircularText(f.textContainer, f.hoverText, Math.max(f.width, f.height) / 2 + 20, f.hoverColor);
